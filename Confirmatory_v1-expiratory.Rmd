---
title: "Gesture kinetics influence expiratory airflow"
description: | 
...
```
This document contains further information on the supporting methods and results of "Arm movements increase acoustic markers of expiratory flow".
...
```{r logeffectsmovementposnegpeaks, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Effects of movement condition of positive and negative peaks in the voice amplitude. Same as the previous plot, but we here log-transformed the amplitude peak values.", fig.height=10}
...
### Is different muscle activity differently affecting amplitude of expiration? {#analysis-2}
...
A model with participant as random intercept (a more complex random slope model did not converge) the different peak muscle activities explained more variance than a base model predicting the overall mean, Change in Chi-Squared (`r printnum(round(pospeaks_bymuscle_comparison[2,1]-pospeaks_bymuscle_comparison[1,1]))`) = `r printnum(pospeaks_bymuscle_comparison$Chisq[2])`, *p* `r paste0("= ", ifelse(pospeaks_bymuscle_comparison[2,8] > .001, round(pospeaks_bymuscle_comparison[2,8], digits = 3), "< .001"))`). The model coefficients are given in Table S\@ref(tab:modelpospeaksbymuscle). Further, Table S\@ref(tab:modelpospeaksbymuscle) shows that peak EMG activity in the infraspinatus leads to statistically reliable increases in positive peaks in the amplitude envelope (with stronger effects).
...
# #only needed when using envdb
# # sub$min_amp_c_around_move <- na_if(sub$min_amp_c_around_move, c(Inf))
# sub <- sub %>% 
#   tidyr::drop_na(min_amp_c_around_move)
# 
# sub$min_amp_c_around_move_abs <- abs(sub$min_amp_c_around_move)

#recycle submin

negpeaks_bymuscle_nullmodel <- lmer(min_amp_log ~ 1 + ( 1| ppn), data = submin)
negpeaks_bymuscle <- lmer(min_amp_log ~ max_erector + max_infra + max_pectoral + max_rectus + (1 | ppn), data = submin)
...
We similarly modeled the variance of the magnitude in negative peaks in the amplitude envelope with muscle activity peaks (see right side of Figure S\@ref(fig:amplitudeEMGpeaks) for graphical results), and observed that such a model did not perform better than a base model predicting the overall mean, Change in Chi-Squared = `r printnum(negpeaks_bymuscle_comparison$Chisq[2])` (`r printnum(round(negpeaks_bymuscle_comparison[2,1]-negpeaks_bymuscle_comparison[1,1]))`), *p* `r paste0("= ", ifelse(negpeaks_bymuscle_comparison[2,8] > .001, round(negpeaks_bymuscle_comparison[2, 8], digits = 3), "< .001"))`. The model coefficients are given in Table S\@ref(tab:modelnegpeaksbymuscle). 
...
Firstly we assess for possible context-dependent interrelationships of posture, muscle activity, and voice, by producing correlation matrices per movement condition (see Tables S\@ref(tab:corrinternalrotation), S\@ref(tab:correxternalrotation), S\@ref(tab:corrextension), S\@ref(tab:corrflexion)). We see that producing different movements can recruit variable interrelationships between muscle, posture and voice. For exhalation, we use the log-transformed amplitude.
...
Figure S\@ref(fig:correlationpostureinternalrotation) visualizes the correlation between posture (max. change in center of pressure) and expiration amplitude (positive peaks in amplitude) that was found for the voicing condition for the internal rotation condition in Table S\@ref(tab:corrinternalrotation).
In the expiration condition, however, there are no correlations between posture and expiration amplitude.
